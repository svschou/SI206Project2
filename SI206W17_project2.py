...
try:
	...
	# reads in data as one long string
...
def find_urls(input_string):
	url_list = re.findall('http://\S+\.\S+|https://\S+\.\S+', input_string)
	return url_list
...
def get_umsi_data():
	...
	# if there is already available data, use cached data
	if "umsi_directory_data" in CACHE_DICTION:
		print("Using cache data: ")

		# grab the html strings from the cache
		for page in CACHE_DICTION["umsi_directory_data"]:
			html_strings.append(page)
	# if not, go grab new data
	else:
		print("Getting new data: ")

		html_page_list = []

		# loop through all the pages in the UMSI directory
		for num in page_num:
			print(num)
		...
		# update cache whenever you grab new data
...
def get_five_tweets(user_string):
	...
	# setting up key to be used in CACHE_DICTION
...
for tweet in five_tweets:
	tweet_url = find_urls(tweet) #a list of urls for each tweet
	# for each url string in the list of urls found in the tweet:
	for url in tweet_url: 
		# append that url string
